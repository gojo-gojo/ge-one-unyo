...
kanshi.sh
    └─ Zabbix エージェント（popular_page.html の更新監視 → Discord 通知）
```
...
記事更新間隔の監視と Brevo（メール）によるアラート送信を行っていた旧スクリプト。cron はコメントアウト済みで、監視は `kanshi.sh` + Zabbix + Discord に移行済み。削除予定。
