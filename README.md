...
del-cache.py + feed_cache.py（5分ごと）
    ├─ WP キャッシュの削除・再生成
    └─ 直近50記事のキャッシュ生成
...
|---|---|---|---|
| `*/5 * * * *` | www-data | `del-cache.py` → `feed_cache.py` | WP キャッシュの削除・再生成、直近50記事のキャッシュ生成 |
| `10,30,50 * * * *` | www-data | `cron_delete-db-files.sh` | 記事削除・メディア整理・人気記事 HTML 生成 |
| `5 */2 * * *` | www-data | `rank.py` | 相互リンクランキング更新 |

5分ごとのキャッシュ処理の実際の cron 行:

```cron
*/5 * * * * www-data python3 /opt/nginx-access/del-cache.py && python3 /opt/nginx-access/feed_cache.py > /dev/null 2>&1
```

ファイル先頭の `POPULAR_PAGE_DOMAIN='gekiyasutokka.com'` は、`del-cache.py`、`feed_cache.py`、`popular_page.py` が参照する環境変数。
...
トップページ・カテゴリページ・カテゴリ別一覧のキャッシュを削除して再生成する。古い個別記事キャッシュ（24 時間超）も削除する。

- **対象ホスト**: 環境変数 `POPULAR_PAGE_DOMAIN`、または引数 `test`、デフォルト `gekiyasutokka.com`
- **キャッシュ先**: `/var/www/nginx/gekiyasutokka.com/wp-content/cache/`
- **HTTP アクセス箇所**:
  - `main_pager()` — トップ `/` ～ `/page/9/`（9ページ）
  - `food_pages()` — カテゴリ各9ページ（ファッション・食品・生活雑貨）
  - `main_pages()` — 上記 HTML から直近3日分の記事 URL を抽出してアクセス

cron では完了後に `feed_cache.py` が続けて実行される（`&&` で連結）。
...
### feed_cache.py — 直近記事のキャッシュ生成

WordPress REST API から直近記事の URL を取得し、1件ずつアクセスして WP キャッシュを生成する。`del-cache.py` の `main_pages()` より軽量で、feed ベースの新着記事向け。

- **API**: `/wp-json/wp/v2/posts?per_page=50&_fields=link`
- **件数**: `POST_LIMIT = 50`（ファイル先頭で変更可能。REST API の上限は100件/リクエスト）
- **対象ホスト**: `del-cache.py` と同じ（`POPULAR_PAGE_DOMAIN` / 引数 `test`）
- **アクセス経路**: `127.0.0.1` 経由（外部からの 403 を回避）

RSS feed（`/feed/`）は WordPress 設定上10件上限のため使用しない。

```bash
python3 /opt/nginx-access/feed_cache.py
# 【feed-cache ...】 article urls: 50
# 【feed-cache ...】 done: 50 urls, slow=0
```
...
`del-cache.py` 以前の bash 版キャッシュ生成スクリプト。cron 未使用。
